...
@section('main')
 ...
 {{ $table_name =  last(explode('/', url()->current())) }}
 ...
 <div class="bg-gray-100 rounded-lg shadow-md p-6">
     ...
     <p class="text-2xl font-bold text-black text-center pb-4 border-b-2 border-b-black">Contenido de la tabla: <span class="uppercase ">{{ $table_name }} </span></p>
     ...
     @else
         ...
         <div class="mb-4 mt-8 ml-16 text-left">
             <a href="{{ url('/') }}" class="bg-white hover:bg-green-500 text-green-500 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">
                 Crear
             </a>
         </div>
                         ...
                         <td class="px-6 py-4 text-gray-900 relative">
                             <div x-data="{ open: false }" class="relative">
                                 ...
                                 <div x-show="open" x-cloak @click.away="open = false"
                                     ...
                                     <ul class="py-1">
                                         <li><a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Editar</a></li>
                                         <li><a href="#" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Borrar</a></li>
                                     </ul>
                                 </div>
                             </div>
                         </td>
         ...
         <div class="flex items-center justify-between mt-6">
             ...
             <div class="flex items-center justify-center mt-6 space-x-2">
                 ...
                 @else
                     <a href="{{ $table_data->previousPageUrl() }}" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Anterior</a>
                 @endif
                 ...
                 @for ($i = $start; $i <= $end; $i++)
                     ...
                     @else
                         <a href="{{ $table_data->url($i) }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">{{ $i }}</a>
                     @endif
                 @endfor

                 @if ($table_data->hasMorePages())
                     <a href="{{ $table_data->nextPageUrl() }}" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Siguiente</a>
                 @else
