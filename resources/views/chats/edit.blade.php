            ...
            <div class="bg-gray-50 px-4 py-3 rounded-t mb-6">
                ...
                <div class="mb-4">
                    ...
                    <select name="participantes[]" id="participante_1" required
                        ...
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ (old('participantes.0', $participantes[0] ?? null) == $usuario->id) ? 'selected' : '' }}>{{ $usuario->usuario }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="participantes[]" id="participante_2" required
                        ...
                        <option value="">Seleccione un usuario</option>
                        @foreach($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" {{ (old('participantes.1', $participantes[1] ?? null) == $usuario->id) ? 'selected' : '' }}>{{ $usuario->usuario }}</option>
                        @endforeach
                    </select>
